...
beforeEach(() => {
  // Restore default env between tests; some tests mutate it.
...
describe('analyzeBrand — happy path with grid capture', () => {
  it('feeds viewport screenshots into analyzeInstagramVisuals and tags igAnalysis with playwright source', async () => {
    mocks.captureInstagramGrid.mockResolvedValue({
      profilePicUrl: 'https://r2.example/profile.jpg',
      viewportShotUrls: [
        'https://r2.example/v00.png',
        'https://r2.example/v01.png',
      ],
      estimatedTilesSeen: 24,
      capturedAt: '2026-04-27T22:00:00.000Z',
      artifactPrefix: 'brand-assets/brand-1/ig-grid/run-1',
    });

    const result = await analyzeBrand({ brandId: 'brand-1', handle: 'humansofny' });

    expect(result.ok).toBe(true);
    expect(mocks.captureInstagramGrid).toHaveBeenCalledWith({
      brandId: 'brand-1',
      handle: 'humansofny',
    });
    ...
    expect(updateCall.igAnalysisJson.gridCapture).toMatchObject({
      source: 'playwright',
      viewportShotUrls: [
        'https://r2.example/v00.png',
        'https://r2.example/v01.png',
